...
class Exp(Function):
  def forward(self, x:LazyBuffer) -> LazyBuffer:
    ...
    k = x.e(BinaryOps.MUL, x.const(1/math.log(2)))
    k = k.e(BinaryOps.CMPLT, x.const(0)).e(TernaryOps.WHERE, k.e(UnaryOps.NEG), k)
    k = x.const(MAX_SHIFT).e(BinaryOps.CMPLT, k).e(TernaryOps.WHERE,  x.const(MAX_SHIFT), k).cast(dtypes.int)
    pow_k = x.const(1).cast(dtypes.int).e(BinaryOps.LSHIFT, k).cast(x.dtype)
    pow_k = x.e(BinaryOps.CMPLT, x.const(0)).e(TernaryOps.WHERE, x.const(1).e(BinaryOps.DIV, pow_k), pow_k)
    y = x.e(BinaryOps.SUB, k.cast(x.dtype).e(BinaryOps.MUL, x.const(math.log(2))))
    exp_y = term = x.const(1)
    for i in range(1, 14):
      term = term.e(BinaryOps.MUL, y)
      exp_y = exp_y.e(BinaryOps.ADD, term.e(BinaryOps.MUL, x.const(1/math.factorial(i))))
    self.ret = x.e(BinaryOps.CMPEQ, x.const(-math.inf)).e(TernaryOps.WHERE, x.const(0), exp_y.e(BinaryOps.MUL, pow_k))
    return self.ret
